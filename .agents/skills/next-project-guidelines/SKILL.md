...
- Body copy and chrome stay in `div` or `span`. Do not use `p`. Chrome includes prices, counts, buttons, tabs, OTP digits, wallet balance, empty-state copy, and header/footer brand text.
- Page, section, and item titles use `AppHeading` from `app/design-system/components/ui/text.tsx`. Do not write raw `h1`–`h6`. Pass the same `text-*` / `font-*` classes the design already uses; `AppHeading` only adds `m-0` and the semantic tag. Browser heading defaults are reset in `app/globals.css`.
- One `level={1}` per page/route. `level={2}` for major sections. `level={3}` for repeated item titles (product cards, cart lines, admin list cards). Nested titles under an `h3` may use `level={4}`. Never put a heading inside a `button` or `a`. Header/footer brand is not a page `h1`.
...
- Visible outline: one `AppHeading level={1}` per page, `level={2}` sections, `level={3}` item titles. Do not heading-ize prices or chrome. Keep `div`/`span` for body copy. Still no `p`. Document title and JSON-LD stay required alongside headings.
...
- When `totalCount` / structure `count` is known, use that exact count and do not invent extra skeletons to fill leftover viewport height. When the total is unknown and the collection is not paginated (`onCapacityChange` / `hasMore` / `onLoadMore` absent), measure one row (wrap/horizontal) or one item (column) only. On entering `/panel/admin`, immediately fetch `GET /api/admin/catalog/structure`. That payload is count-only: `products`, `orders`, `users`, `banners`, plus `{ id, count }[]` for showcases, category groups, and brand groups, plus `storefront: { home, categories, products }` for the layout tab. The layout tab is not paginated: pass that exact count, do not pass `onCapacityChange`, and size row chrome (title, type, order input, tabs, helper text) to content so loading and loaded heights match. Do not stretch the order input to full row width. On entering `/panel/user` after login, immediately fetch `GET /api/user/structure`. That payload is count-only: `orders`, `discounts`, and `unseenDiscounts`. Order and discount tab skeletons use `min(count, viewportFit)`. The discounts tab badge shows `unseenDiscounts` only; opening that tab POSTs `{ seenDiscounts: true }` so the badge stays at 0 until a new active code is issued. Do not put titles, sort orders, layouts, or records in these structure APIs. Do not wrap the whole tab in an extra `skeleton-structure` around the collection. Pass `onCapacityChange` only after structure counts exist. A `totalCount` of `0` must mark the section loaded and show the empty state.
- Feed `DynamicLoadingCollection.onCapacityChange` into the query/API limit only for paginated collections. Keep the query disabled while capacity is zero, include capacity in the query key when it changes the response, fetch the first viewport-sized batch only, then use the central sentinel to request one further viewport-sized batch as scrolling reaches it. Do not render the load-more sentinel when the list is not paginated. Horizontal storefront showcase rails are not paginated: fetch the structure `productCount` (or showcase `limit`), do not pass `onCapacityChange`, keep each card at intrinsic width (`shrink-0 w-max`, never `max-w-full` on the rail item), and let `overflow-x-auto` native scrolling show the rest. Mouse drag may scroll the rail; touch/pointer must not call `preventDefault` on `pointerdown` or native swipe dies.
